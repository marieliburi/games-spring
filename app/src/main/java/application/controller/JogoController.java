...
public class JogoController {
    ...
    public String insert(
        ...
        for (long p : idsPlataformas) {
            ...
            plataforma.ifPresent(value -> jogo.getPlataformas().add(value));
        }
    ...
    public String update(
        ...
        if (jogo.isPresent()) {
            Jogo j = jogo.get();
            j.setTitulo(titulo);
            j.setCategoria(categoriaRepo.findById(idCategoria).get());

            // Atualizar plataformas
            Set<Plataforma> updatePlataforma = new HashSet<>();
            for (long p : idsPlataformas) {
                Optional<Plataforma> plataforma = plataformaRepo.findById(p);  // corrigido aqui
                plataforma.ifPresent(updatePlataforma::add);
            }
            j.setPlataformas(updatePlataforma);
